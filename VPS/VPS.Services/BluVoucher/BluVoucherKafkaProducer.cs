...
{
    public class BluVoucherKafkaProducer : IBluVoucherKafkaProducer
    {
        ...
        public BluVoucherKafkaProducer(IVpsKafkaProducer kafkaProducer,
        ...
        public async Task Produce(BluVoucherRedeemRequest bluVoucherRedeemRequest, VoucherType voucherType, RedeemOutcome outcome)
        {
            //Produce in Kafka and return custom message
            var quePayload = new KafkaQueuePayload<BluVoucherRedeemRequest>
            {
                VoucherRedeemRequest = bluVoucherRedeemRequest,
            ...
            try
            {
                ...
                AsyncRetryPolicy<DeliveryResult<Null, string>?> _retryPolicy = Policy
                    ...
                    .WaitAndRetryAsync(
                        ...
                        {
                            noOfAttempts++;
                            _log.LogInformation(bluVoucherRedeemRequest.VoucherNumber, "Producing message on Kafka: {message}. Retry Attempt: {noOfAttempts}",
                ...
                _log.LogInformation(bluVoucherRedeemRequest.VoucherNumber, "Produce on Kafka response: {response}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                    JsonConvert.SerializeObject(kafkaResponse));

                if (kafkaResponse == null || kafkaResponse.Status != PersistenceStatus.Persisted)
                {
                    _log.LogInformation(bluVoucherRedeemRequest.VoucherNumber, "Failed to Persist message on Kafka: {message}.",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                                 kafkaMessage);

                    //Log to sql table for batch processing, should be flagged that its from Producer
                    _log.LogInformation(bluVoucherRedeemRequest.VoucherNumber, "Inserting voucher {voucherNumber} to VoucherBatchProcessing",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                        bluVoucherRedeemRequest.VoucherNumber);

                    await InsertToBatchTable(bluVoucherRedeemRequest, quePayload, kafkaMessage);
                    ...
                    {
                        ClientId = bluVoucherRedeemRequest.ClientId.ToString(),
            ...
            {
                _log.LogError(ex, bluVoucherRedeemRequest.VoucherNumber, "Failed to produce message on Kafka with error message: {message}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message);

                //Log to sql table for batch processing, should be flagged that its from Producer
                _log.LogInformation(bluVoucherRedeemRequest.VoucherNumber, "Inserting voucher {voucherNumber} to VoucherBatchProcessing",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty, bluVoucherRedeemRequest.VoucherNumber);
                await InsertToBatchTable(bluVoucherRedeemRequest, quePayload, kafkaMessage);
        ...
        private async Task InsertToBatchTable(BluVoucherRedeemRequest bluVoucherRedeemRequest, KafkaQueuePayload<BluVoucherRedeemRequest> quePayload, string kafkaMessage)
        {
            ...
            {
                _log.LogError(ex, bluVoucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message);
                _log.LogCritical(ex, bluVoucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message} and payload: {payload}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message, kafkaMessage);
