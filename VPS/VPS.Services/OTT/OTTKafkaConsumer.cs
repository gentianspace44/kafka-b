...
{
    public class OttKafkaConsumer : IVpsKafkaConsumer
    {

        private readonly ILoggerAdapter<OttKafkaConsumer> _logger;
        ...
        public OttKafkaConsumer(IClientBalanceService clientBalanceService,
            ILoggerAdapter<OttKafkaConsumer> logger,
        ...
        public async Task<bool> ConsumeAndProcessTransaction(string message)
        {
            try
            {
                ...
                var nullablePayload = JsonConvert.DeserializeObject<KafkaQueuePayload<OttVoucherRedeemRequest>>(message);
                ...
                KafkaQueuePayload<OttVoucherRedeemRequest> requestObject = nullablePayload!;
                ...
                AsyncRetryPolicy<CreditOnSyxResult> _retryPolicy = Policy
                    ...
                    .WaitAndRetryAsync(
                        ...
                        {
                            noOfAttempts++;
                            _logger.LogInformation(requestObject.VoucherRedeemRequest?.VoucherNumber, "Calling CreditOnSyx on SyxApi with payload: {request}. Retry Attempt: {noOfAttempts}",
                             MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                                 message, noOfAttempts );
                        }
                    );
                ...
                if (!creditOnSyxResult.IsSuccess)
                {
                    ...
                    try
                    {
                        ...
                        {
                            _logger.LogError(requestObject.VoucherRedeemRequest?.VoucherNumber, "Missing VoucherID on RedeemOutcome: {VoucherNumber}."
                                , MethodBase.GetCurrentMethod()?.Name ?? string.Empty, requestObject.VoucherRedeemRequest?.VoucherNumber ?? "");
                        ...
                        else
                        {
                            _logger.LogError(requestObject.VoucherRedeemRequest?.VoucherNumber, "Failed to Parse VoucherReference on Consumer: {VoucherReference}."
                                , MethodBase.GetCurrentMethod()?.Name ?? string.Empty, requestObject.VoucherRedeemRequest?.VoucherReference ?? "" );
        ...
        private async Task<CreditOnSyxResult> CreditOnSyx(KafkaQueuePayload<OttVoucherRedeemRequest> requestObject)
        {
            ...
            {
                _logger.LogInformation(requestObject.VoucherRedeemRequest.VoucherNumber, "{voucherPrefix} voucher with pin {voucherNumber} - Message: {outcomeMessage}, Amount: {voucherAmount}, BalanceAvailable: {balanceAvailable}",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                     voucherPrefix, requestObject.VoucherRedeemRequest.VoucherNumber, syxCreditOutcome.OutcomeMessage, requestObject.RedeemOutcome.VoucherAmount, syxCreditOutcome.BalanceAvailable);
            ...
            if (syxCreditOutcome.OutComeTypeId == (int)SyxCreditOutcome.EligibleForBonus)
            {
                ...
                _logger.LogInformation(requestObject.VoucherRedeemRequest.VoucherNumber, "{voucherPrefix} voucher with pin {voucherNumber} - Message: {outcomeMessage}, Amount: {voucherAmount}, Bonus: {redeemedBonusAmount}, BalanceAvailable: {balanceAvailable}",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                     voucherPrefix, requestObject.VoucherRedeemRequest.VoucherNumber, syxCreditOutcome.OutcomeMessage, requestObject.RedeemOutcome.VoucherAmount, bonusResult.RedeemedBonusAmount, bonusResult.BalanceAvailable);
            ...
            _logger.LogError(requestObject.VoucherRedeemRequest.VoucherNumber, null, "{voucherPrefix} voucher with pin {voucherNumber} - Message: {outcomeMessage}, Amount: {voucherAmount}, BalanceAvailable: {balanceAvailable}",
                MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                 voucherPrefix, requestObject.VoucherRedeemRequest.VoucherNumber, syxCreditOutcome.OutcomeMessage, requestObject.RedeemOutcome.VoucherAmount, syxCreditOutcome.BalanceAvailable);
        ...
        private bool IsPayloadValid(KafkaQueuePayload<OttVoucherRedeemRequest>? queuePayload)
        {
