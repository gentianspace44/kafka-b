...
namespace VPS.Services.OTT
{
    public class OttKafkaProducer : IOttKafkaProducer
    {
        ...
        public OttKafkaProducer(IVpsKafkaProducer kafkaProducer,
            IOptions<KafkaQueueConfiguration> queueConfiguration,
            ILoggerAdapter<OttKafkaProducer> log,
        ...
        public async Task Produce(OttVoucherRedeemRequest oTTVoucherRedeemRequest, VoucherType voucherType, RedeemOutcome outcome)
        {
            //Produce in Kafka and return custom message
            var quePayload = new KafkaQueuePayload<OttVoucherRedeemRequest>
            {
                VoucherRedeemRequest = oTTVoucherRedeemRequest,
            ...
            try
            {
                ...
                AsyncRetryPolicy<DeliveryResult<Null, string>?> _retryPolicy = Policy
                    ...
                    .WaitAndRetryAsync(
                        ...
                        {
                            noOfAttempts++;
                            _log.LogInformation(oTTVoucherRedeemRequest.VoucherNumber, "Producing message on Kafka: {message}. Retry Attempt: {noOfAttempts}",
                ...
                _log.LogInformation(oTTVoucherRedeemRequest.VoucherNumber, "Produce on Kafka response: {response}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                    JsonConvert.SerializeObject(kafkaResponse));

                if (kafkaResponse == null || kafkaResponse.Status != PersistenceStatus.Persisted)
                {
                    _log.LogInformation(oTTVoucherRedeemRequest.VoucherNumber, "Failed to Persist message on Kafka: {message}.",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                               kafkaMessage );
                    //Log to sql table for batch processing, should be flagged that its from Producer
                    _log.LogInformation(oTTVoucherRedeemRequest.VoucherNumber, "Inserting voucher {voucherNumber} to VoucherBatchProcessing", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, oTTVoucherRedeemRequest.VoucherNumber);

                    await InsertToBatchTable(oTTVoucherRedeemRequest, quePayload, kafkaMessage);
                    ...
                    {
                        ClientId = oTTVoucherRedeemRequest.ClientId.ToString(),
            ...
            {
                _log.LogError(ex, oTTVoucherRedeemRequest.VoucherNumber, "Failed to produce message on Kafka with error message: {message}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty,  ex.Message );
        ...
        private async Task InsertToBatchTable(OttVoucherRedeemRequest oTTVoucherRedeemRequest, KafkaQueuePayload<OttVoucherRedeemRequest> quePayload, string kafkaMessage)
        {
            ...
            {
                _log.LogError(ex, oTTVoucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message);
                _log.LogCritical(ex, oTTVoucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message} and payload: {payload}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message, kafkaMessage);
