...
{
    public class EasyLoadKafkaProducer : IEasyLoadKafkaProducer
    {

        private readonly IVpsKafkaProducer _kafkaProducer;
        private readonly KafkaQueueConfiguration _queueConfiguration;
        private readonly ILoggerAdapter<EasyLoadKafkaProducer> _log;
        ...
        public EasyLoadKafkaProducer(IVpsKafkaProducer kafkaProducer,
            IOptions<KafkaQueueConfiguration> queueConfiguration,
            ILoggerAdapter<EasyLoadKafkaProducer> log,
        ...
        public async Task Produce(EasyLoadVoucherRedeemRequest voucherRedeemRequest, VoucherType voucherType, RedeemOutcome outcome)
        {
            //Produce in Kafka and return custom message
            var quePayload = new KafkaQueuePayload<EasyLoadVoucherRedeemRequest>
            {
            ...
            {
                _log.LogError(ex, voucherRedeemRequest.VoucherNumber, "Failed to produce message on Kafka with error message: {message}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                     ex.Message );
        ...
        private async Task InsertToBatchTable(EasyLoadVoucherRedeemRequest voucherRedeemRequest, KafkaQueuePayload<EasyLoadVoucherRedeemRequest> quePayload, string kafkaMessage)
        {
            ...
            {
                _log.LogError(ex, voucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message);
                _log.LogCritical(ex, voucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message} and payload: {payload}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message);
