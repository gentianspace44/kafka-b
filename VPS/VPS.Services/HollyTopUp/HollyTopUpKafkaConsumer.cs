...
{
    public class HollyTopUpKafkaConsumer : IVpsKafkaConsumer
    {

        private readonly ILoggerAdapter<HollyTopUpKafkaConsumer> _logger;
        ...
        public HollyTopUpKafkaConsumer(IClientBalanceService clientBalanceService,
            ILoggerAdapter<HollyTopUpKafkaConsumer> logger,
        ...
        public async Task<bool> ConsumeAndProcessTransaction(string message)
        {
            ...
            try
            {
                ...
                var nullablePayload = JsonConvert.DeserializeObject<KafkaQueuePayload<HollyTopUpRedeemRequest>>(message);
                ...
                KafkaQueuePayload<HollyTopUpRedeemRequest> requestObject = nullablePayload!;
                ...
                if (!creditOnSyxResult.IsSuccess)
                {
                    ...
                    try
                    {
                        ...
                        {
                            _logger.LogError(requestObject.VoucherRedeemRequest?.VoucherNumber, "Failed to Parse VoucherReference on Consumer: {VoucherReference}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, requestObject.VoucherRedeemRequest?.VoucherReference ?? "");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(ex, requestObject.VoucherRedeemRequest?.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message}.",
                            MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                            ex.Message);
                        _logger.LogCritical(ex, requestObject.VoucherRedeemRequest?.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message} with payload: {payload}.",
                            MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                            ex.Message, message);
            ...
            {
                _logger.LogError(ex, "ConsumeAndProcessTransaction", "Error in ClientBalanceSyxTransactionConsumerService: {ExMessage}",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                    ex.Message.ToString());
                _logger.LogCritical(ex, "ConsumeAndProcessTransaction", "Error in ClientBalanceSyxTransactionConsumerService: {ExMessage} with payload: {payload}.",
                    MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                    ex.Message, message);
                throw;
            }
        ...
        private async Task<CreditOnSyxResult> CreditOnSyx(KafkaQueuePayload<HollyTopUpRedeemRequest> requestObject)
        {
        ...
        private bool IsPayloadValid(KafkaQueuePayload<HollyTopUpRedeemRequest>? queuePayload)
        {
