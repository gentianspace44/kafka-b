...
namespace VPS.Services.Flash
{
    ...
    public class FlashKafkaProducer : IFlashKafkaProducer
    {
        ...
        public FlashKafkaProducer(IVpsKafkaProducer kafkaProducer,
            IOptions<KafkaQueueConfiguration> queueConfiguration,
            ILoggerAdapter<FlashKafkaProducer> log,
        ...
        public async Task Produce(FlashRedeemRequest voucherRedeemRequest, VoucherType voucherType, RedeemOutcome outcome)
        {
            //Produce in Kafka and return custom message
            var quePayload = new KafkaQueuePayload<FlashRedeemRequest>
            {
            ...
            try
            {
                ...
                AsyncRetryPolicy<DeliveryResult<Null, string>?> _retryPolicy = Policy
                    ...
                    .WaitAndRetryAsync(
                        ...
                        {
                            noOfAttempts++;
                            _log.LogInformation(voucherRedeemRequest.VoucherNumber, "Producing message on Kafka: {message}. Retry Attempt: {noOfAttempts}",
                ...
                _log.LogInformation(voucherRedeemRequest.VoucherNumber, "Produce on Kafka response: {response}.",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                    JsonConvert.SerializeObject(kafkaResponse) );

                if (kafkaResponse == null || kafkaResponse.Status != PersistenceStatus.Persisted)
                {
                    _log.LogInformation(voucherRedeemRequest.VoucherNumber, "Failed to Persist message on Kafka: {message}.",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty,
                                 kafkaMessage );
                    //Log to sql table for batch processing, should be flagged that its from Producer
                    _log.LogInformation(voucherRedeemRequest.VoucherNumber, "Inserting voucher {voucherNumber} to VoucherBatchProcessing",
                        MethodBase.GetCurrentMethod()?.Name ?? string.Empty, voucherRedeemRequest.VoucherNumber);

                    await InsertToBatchTable(voucherRedeemRequest, quePayload, kafkaMessage);
                    ...
                    {
                        ClientId = voucherRedeemRequest.ClientId.ToString(),
            ...
            {
                _log.LogError(ex, voucherRedeemRequest.VoucherNumber, "Failed to produce message on Kafka with error message: {message}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message );

                //Log to sql table for batch processing, should be flagged that its from Producer
                _log.LogInformation(voucherRedeemRequest.VoucherNumber, "Inserting voucher {voucherNumber} to VoucherBatchProcessing", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, voucherRedeemRequest.VoucherNumber);
                await InsertToBatchTable(voucherRedeemRequest, quePayload, kafkaMessage);
        ...
        private async Task InsertToBatchTable(FlashRedeemRequest voucherRedeemRequest, KafkaQueuePayload<FlashRedeemRequest> quePayload, string kafkaMessage)
        {
            ...
            {
                _log.LogError(ex, voucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message );
                _log.LogCritical(ex, voucherRedeemRequest.VoucherNumber, "Failed to insert record message on VoucherBatchProcessing with error message: {message} and payload: {payload}.", MethodBase.GetCurrentMethod()?.Name ?? string.Empty, ex.Message, kafkaMessage );
